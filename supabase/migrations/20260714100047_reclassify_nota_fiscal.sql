-- reclassify_charge learns p_nota_fiscal (Gabriel 2026-07-14): aprovar um
-- documento de e-mail p/ Pagamentos exige classificar o método e, se boleto,
-- preencher a Nota Fiscal (obrigatória) — a NF é gravada no mesmo patch da
-- reclassificação. DROP first (adding a parameter would OVERLOAD, not replace).
-- Body identical to migration 36 except the p_nota_fiscal param + its coalesce.
...
begin
  ...
  set kind               = p_kind,
      ...
      nota_fiscal        = coalesce(p_nota_fiscal, nota_fiscal),
      notes              = coalesce(p_notes, notes),
      billing_account_id = v_ba,
      station_id         = v_station,
      dedupe_key         = coalesce(v_new_dedupe, dedupe_key),
...
revoke execute on function charging.reclassify_charge(
  uuid, charging.charge_kind, date, numeric, numeric, jsonb, integer, integer,
  text, text, charging.payment_method, text, text, text, text, text, text, date, text
) from public, anon;
